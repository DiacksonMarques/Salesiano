/*
	Template Name: Cornike - backusiness HTML5 Template
	Author: Tripples
...
jQuery(function($) {
    ...
    if ($('#map').length > 0) {
        ...
        .infowindow({
            position: contactmap,
            content: "Cólegio Salesiano Juazeiro(São João Bosco)"
        })
...
function mascTel(texto) {
    if (event.KeyCode !== 8) {
        if (texto.length === 2) {
            document.getElementById("telefone").value = "(" + texto + ")" + " 9";
...
/*botão mascara Portal*/
function mascPORTAL(texto) {
    if (event.KeyCode !== 8) {
        if (texto.length === 3 || texto.length === 7) {
            document.getElementById("portal").value = texto + ".";
        }
    }
}
